...
## 2026-07-04 — Observe-learn fixes + learner weekly report BUILT, AUDITED, DEPLOYED LIVE

**Decided by:** Mark ("happy for you to deploy it all... need it to be healthy. If it's not, troubleshoot and self heal") + Claude Code (overnight multi-agent build).
**What shipped (prod `cbc06d0` → `7500a9e`, 40 commits):** all five review build prompts plus a sixth added by Mark's ask:
1. Suggestion safety gates: min-price floor (EngineSnapshot.min → settings min → 180d rate_states low → floorUnknown), event shield via shared `eventAdjustmentForDate`, anti-compounding (approved/applied skip + 14d 25% cumulative cap), occupancy-scaled trigger, multi-unit awareness, blocked-by-gate counters in the readout, listing names in output.
2. Tenant-lifecycle scheduling: run-time reconcile repeatable (05:15 London) enrols tenants created after boot and prunes dead ids (fixes the two silently unobserved recreated tenants); per-tenant estate health + learning-starvation matrix in the readout; new `ObserveLearningLedger` table records per-run sample counts and null-reasons.
3. Suggestion history + shadow scoring: supersede instead of delete, shadow suggestions from day 1 for ungraduated tenants, weekly ghost scorer (booked_no_action / expired_empty / cancelled_after_booking with realised-rate ratios), calibration report and method-agreement (experimental) sections in the readout.
4. Honest regret + rules + global doc: regret over settled nights only with a seasonal baseline, `held_too_low` null (not 0) without min data, artefact rules retired (`tolerates_empty_premium` tautology, `below_min_short_window` renamed and the below-min permission downgraded to observation-only), global methodology rebuilt recompute-from-latest with equal client weighting (deleted-tenant ghosts evicted).
5. Drop dose-response mining (analysis-only): 20,288 historical drop episodes, 1,204 settled treated nights. Headline: 4–7d-lead drops beat matched controls at every size (+11 to +17pp fill); 15–30d drops underperform with doubled cancellations. Report: `observe-reports/drop-outcomes-prod-2026-07-03.md`.
6. Learner weekly report: one plain-English email per ISO week after the Monday settle covering every client (health, "what we would have done vs what happened" calibration, safety-gate blocks, blind spots, coming up), `.email-sent` guard, email failure never fails the settle. Example: `observe-reports/learner-weekly-example-2026-07-06.html`.

**Audits:** three independent fresh-context auditors (code, behavioural, final) all ruled SAFE, zero blockers; behavioural auditor exercised the Fleadh event shield, min clamp, shadow exclusion, regret fixes and lifecycle reconcile against seeded data and reproduced the prod mining report byte-identical. One audit finding (ghost scorer counting ownerstay/≤£5 nights as bookings) was fixed and re-verified same night. Audit files: `reviews/observe-learn-2026-07/BUILD-AUDIT-*.md`.

**Deploy evidence:** two additive migrations applied via `prisma migrate deploy` (prod now 24, latest `20260704040000_observe_learning_ledger`); web + worker both RUNNING on `7500a9e`; baseline vs post: `/` 200 identical render, `/login` 200, readout 200 with the new estate/starvation shape (6 tenants); worker boot log shows all five workers registered incl. hourly rate-copy (6 tenants, 12 stale repeatables pruned) and the new observe reconcile (05:15).
**Rollback (one line):** `git push --force-with-lease origin backup/prod-live-observe-fixes:main` (= `cbc06d0`), then redeploy + `signals-worker` restart. The two migrations are additive and safe to leave in place.
**Status:** LIVE. First reconciled observe run 05:15/05:30 London 2026-07-04; first weekly report Monday 2026-07-06 after the settle; graduation readout 2026-07-28 now ships gated suggestions and honest rules.

## 2026-07-04 (daytime) — Learning granularity shipped: cohort hierarchy, actual-paid signal, richer dimensions (DEPLOYED LIVE)

**Decided by:** Mark ("Yep build prompt needed... Group is good... Size of property... any other sub group... same as before get app to healthy state - commit push and deploy") + Claude Code.
**Trigger:** granularity audit (`reviews/observe-learn-2026-07/07-learning-granularity.md`) proved the flat tenant-grain trigger miscalibrates: group:Argo median booking lead 54d vs group:St James 3d on the same tenant; LF's ~200 block units drowned its 50 flats in the occupancy scaler.
**What shipped (prod `037e566` → `05a9bdd`, 15 commits, build prompt `BUILD-PROMPTS/07-learning-granularity-hierarchy.md`):**
1. **Cohort grain engine** (`src/lib/observe/cohorts.ts`): crossover cohort memberships (listing, group: tags, size band, city, stock type, tenant); lead-curve and DOW-occupancy ladders resolve most-specific-first with prod-calibrated sample gates (e.g. `GROUP_CURVE_MIN_BOOKINGS=60`); the suggestion trigger and occupancy scaling consume the resolved cohort, and every suggestion records `{rung, cohortKey, n}` provenance. Per-client ladders deliberately terminate at tenant rung (a global rung inside per-client generation would leak cross-tenant data).
2. **Actual-paid promo signal** (`src/lib/observe/actual-paid.ts`): paid nightly (from synced Hostaway reservation data; `src/lib/hostaway/**` untouched) vs listed rate near booking (RateChange/RateState). Learned per channel + cohort as learning #8 (`promo_gap`); ghost scores carry `paidVsListedGapPct` + `heavyPromo`; calibration separates promo-filled wins from genuine no-intervention wins. Note: channel medians embed structural VAT/fee wedges (booking.com ~0.74 paid/listed vs airbnb ~0.99), so heavy-promo = gap ≥ channel median + 0.15; reports quote the heavy-promo SHARE, never the raw wedge.
3. **Pickup velocity finally measured** (`src/lib/observe/pickup.ts`): weekly settle computes moved-vs-control 7-day pickup on old-enough PeerControl rows; ~400 of 1,350 backlog controls become measurable at the 2026-07-06 settle, draining in 2-3 settles.
4. **Calibration + dose-response re-cut by group and size band** (min-n suppression), **GlobalMethodology stratified per city** (Belfast = the one in-house market grain; anonymisation whitelist intact, required leak test still passing).
5. **Weekly report new sections:** "How your groups book differently" (plain host language), "Bookings won by big discounts", cohort pattern changes week over week. Example artefact re-rendered.

**Audits:** two independent fresh-context auditors (code + behavioural), both SAFE, zero blockers; behavioural auditor proved on seeded data that the same empty night fires under a late-booking group curve and stays silent under an early-booking one, gates flip at exact boundaries, no cross-tenant pollution, promo gap excludes (never fabricates) missing rate observations. Audit files: `BUILD-AUDIT-07-code.md`, `BUILD-AUDIT-07-behaviour.md`.
